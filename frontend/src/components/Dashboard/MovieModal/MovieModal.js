...
const MovieModal = (props) => {
  ...
  const getMovie = useCallback(async () => {
    ...
    const movieResponse = await axios.post(
      "https://mern-movie-app.onrender.com/movie",
      {
        _id: ctx.appState.userDetails._id,
        id: ctx.appState.movieModal.movieId,
      },
      {
        headers: {
          _id: ctx.appState.userDetails._id,
  ...
  const starClickHandler = async () => {
    if (!starred) {
      const starResponse = await axios.post(
        "https://mern-movie-app.onrender.com/star",
        { _id: ctx.appState.userDetails._id, id: movieState.id },
        {
          headers: {
            _id: ctx.appState.userDetails._id,
            token: ctx.appState.userDetails.token,
      ...
      const starResponse = await axios.post(
        "https://mern-movie-app.onrender.com/unstar",
        { _id: ctx.appState.userDetails._id, id: movieState.id },
        {
          headers: {
            _id: ctx.appState.userDetails._id,
      ...
      {!modalError.errorState && !modalLoader && (
        <React.Fragment>
          <div className="poster">
            <img
              src={
                !movieState.poster || movieState.poster == ""
                  ? "https://img-premium.flaticon.com/png/512/1823/premium/1823458.png?token=exp=1632477527~hmac=7bfcd24f58a0f9b1283b10187cc522e8"
                  : movieState.poster
              }
          ...
          <div className="movie-details">
            ...
            <div className="movie-details-top">
              <div className="movie-details-info">
                ...
                <p className="movie-details-avg">
                  <span>{movieState.vote_average}</span>&nbsp;/&nbsp;10 ||{" "}
                  {movieState.vote_count} votes
                </p>
              </div>
              <div
                className={`star ${starred ? "starred" : "unstarred"}`}
